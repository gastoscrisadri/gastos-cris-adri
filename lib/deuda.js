...
/**
 * Lo apuntado DESPUÉS del último cierre. Sin cierre, todo.
 *
 * Se mira cuándo se APUNTÓ (created_at), no la fecha del gasto. Si alguien
 * apunta mañana una cena de la semana pasada, ese dinero no estaba en la
 * cuenta cuando se cerró, así que tiene que contar.
 */
export function desdeElCierre(transacciones, desdeCierre = null) {
  return desdeCierre
    ? transacciones.filter(t => t.created_at && t.created_at > desdeCierre)
    : transacciones
}

/**
 * Los gastos que entran en la cuenta de los dos desde el último cierre.
 *
 * Está aquí, y no escrito a mano en la pantalla, porque la tarjeta de "quién
 * puso el dinero" enseña la lista y la cuenta enseña el total: si cada una se
 * filtrara por su cuenta, acabarían diciendo cosas distintas. Ya pasó una vez.
 *
 * Ojo con un matiz que despista: entran también los gastos "a cargo de" uno
 * solo. Los ven los dos y alguien ha puesto ese dinero, así que mueven la
 * cuenta aunque luego se le carguen enteros a una persona.
 */
export function gastosComunesDesde(transacciones, desdeCierre = null) {
  return desdeElCierre(transacciones, desdeCierre)
    .filter(t => t.tipo === 'gasto' && t.comun !== false && !esLiquidacion(t))
}

export function calcularDeuda(transacciones, cuentas, miParte, desdeCierre = null) {
  const desde = desdeElCierre(transacciones, desdeCierre)
  const comunes = gastosComunesDesde(transacciones, desdeCierre)
  ...
  // Los ajustes también salen de la lista ya filtrada: un pago que se hizo
  // ANTES del cierre ya está saldado y no debe seguir moviendo el saldo.
  desde.filter(esLiquidacion).forEach(t => {
