...
export function calcularDeuda(transacciones, cuentas, miParte, desdeCierre = null) {
  // Si hay un cierre, la cuenta empieza de cero en ese momento: solo entra lo
  // apuntado DESPUÉS. Sin cierre (desdeCierre = null) no se filtra nada y el
  ...
  const desde = desdeCierre
    ? transacciones.filter(t => t.created_at && t.created_at > desdeCierre)
    : transacciones

  const comunes = desde.filter(t => t.tipo === 'gasto' && t.comun !== false && !esLiquidacion(t))
